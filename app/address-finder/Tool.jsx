...
export default function Tool() {
  ...
  const StatusBox = ({ s }) => {
    if (s.type === 'loading') return <div className="bg-electric-500/10 border border-electric-400/30 text-electric-200 rounded-lg p-3 text-sm mt-3 flex items-center gap-2"><span className="w-4 h-4 rounded-full border-2 border-electric-300 border-t-transparent animate-spin" />{s.msg}</div>;
    ...
    <>
      <section className="glass rounded-2xl p-6 mb-4">
        <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
          ...
          <span className="text-[10px] uppercase tracking-wider text-slate-400 bg-white/5 px-2 py-1 rounded">Forward geocoding</span>
        </div>
        ...
        {addrResult && (
          <dl className="grid sm:grid-cols-2 gap-2 mt-3 text-sm">
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Lat</dt><dd className="font-mono">{addrResult.lat.toFixed(6)}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Lon</dt><dd className="font-mono">{addrResult.lon.toFixed(6)}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2 sm:col-span-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Display name</dt><dd className="text-xs">{addrResult.name}</dd></div>
          </dl>
        )}
      </section>
      ...
      <section className="glass rounded-2xl p-6 mb-4">
        <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
          ...
          <span className="text-[10px] uppercase tracking-wider text-slate-400 bg-white/5 px-2 py-1 rounded">Reverse geocoding</span>
        </div>
        <div className="flex flex-wrap gap-2">
        ...
        {revResult && (
          <dl className="grid sm:grid-cols-2 gap-2 mt-3 text-sm">
            <div className="bg-white/5 border border-white/10 rounded-md p-2 sm:col-span-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Full address</dt><dd className="text-xs">{revResult.full}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Road</dt><dd className="text-sm">{revResult.road}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">City</dt><dd className="text-sm">{revResult.city}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Country</dt><dd className="text-sm">{revResult.country}</dd></div>
            <div className="bg-white/5 border border-white/10 rounded-md p-2"><dt className="text-[10px] uppercase tracking-wider text-slate-400">Postal</dt><dd className="text-sm">{revResult.postal}</dd></div>
          </dl>
        )}
      </section>

      {mapPos && (
        <div className="h-[380px] rounded-2xl overflow-hidden ring-1 ring-white/10">
